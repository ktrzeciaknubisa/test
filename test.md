...
* `options` {Object} [optional]
    * `force` {Boolean} - forces reparsing `process.argv`
...
Each parsed argument has the following members:
...
* `valueSep` {String} - a separator used in an argument to provide a value, for example `=`, `:` and space: `--arg1=str1`, `-c:3`, `-l 0`
* `prefix` {String} - this may be one of the following: `--`, `-` and `/` on Windows platforms
* `type` {String} - this is evaluated value of `typeof value`
...
Beware, that it may be `0`, so it should always not be checked with e.g. `if (!arg.asInt) {}`.
...
or [JX_ARG_SEP](jxcore-utils.html#jxargsep) this function return an array with separated parts (split).
