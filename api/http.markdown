...
The HTTP interfaces in JXcore are designed to support many features
...
In order to support the full spectrum of possible HTTP applications, JXcore's
...
And JXcore does not check whether Content-Length and the length of the body
...
`response.write()` is called, JXcore assumes you're going to be streaming
...
JXcore maintains several connections per server to make HTTP requests.
...
* Sending a 'Connection: keep-alive' will notify JXcore that the connection to
...
Since most requests are GET requests without bodies, JXcore provides this
...
to become free the socket is closed. This means that JXcore's pool has the
...
Note: JXcore does not check whether Content-Length and the length of the body
which has been transmitted are equal or not.
...
`require('url').parse(request.url)`.  Example:
    ...
    jx> require('url').parse('/status?name=ryan')
...
`true` as the second argument to `require('url').parse`.  Example:
    ...
    jx> require('url').parse('/status?name=ryan', true)
    { href: '/status?name=ryan',
      search: '?name=ryan',
      query: { name: 'ryan' },
      pathname: '/status' }
