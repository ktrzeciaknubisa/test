...
unhandled exception means your application - and by extension JXcore itself -
...
- `SIGUSR1` is reserved by JXcore to start the debugger.  It's possible to
...
- `SIGTERM` and `SIGINT` have default handlers on non-Windows platforms that resets
  ...
  (JXcore will no longer exit).
- `SIGPIPE` is ignored by default, it can have a listener installed.
- `SIGHUP` is generated on Windows when the console window is closed, and on other
  platforms under various similar conditions, see signal(7). It can have a
  listener installed, however JXcore will be unconditionally terminated by Windows
  about 10 seconds later. On non-Windows platforms, the default behaviour of
  `SIGHUP` is to terminate JXcore, but once a listener has been installed its
...
- `SIGKILL` cannot have a listener installed, it will unconditionally terminate
  JXcore on all platforms.
- `SIGSTOP` cannot have a listener installed.

Note that Windows does not support sending Signals, but JXcore offers some
...
`process.stderr` and `process.stdout` are unlike other streams in JXcore in
...
To check if JXcore is being run in a TTY context, read the `isTTY` property
on `process.stderr`, `process.stdout`, or `process.stdin`:

    $ jx -p "Boolean(process.stdin.isTTY)"
    true
    $ echo "foo" | jx -p "Boolean(process.stdin.isTTY)"
    false

    $ jx -p "Boolean(process.stdout.isTTY)"
    true
...
'jx', the second element will be the name of the JavaScript file.  The
...
This will generate:
    ...
    $ jx process-2.js one two=three four
    0: jx
    1: /Users/mjr/work/jx/process-2.js
    2: one
    3: two=three
    4: four
...
This is the set of JXcore-specific command line options from the
executable that started the process.  These options do not show up in
`process.argv`, and do not include the jx executable, the name of
...
and process.argv:
    ...
    ['/usr/local/bin/jx', 'script.js', '--version']


## process.abort()

This causes JXcore to emit an abort. This will cause JXcore to exit and
...
The shell that executed JXcore should see the exit code as 1.
...
if the effective group ID is included but JXcore ensures it always is.
...
A property exposing version strings of JXcore and its dependencies.
...
that were used to compile the current jx executable. This is the same as
...
See [Signal Events](#process-signal-events) and kill(2) for more information.
...
[Signal Events](#process-signal-events).
...
Returns an object describing the memory usage of the JXcore process
measured in bytes.
...
In order to avoid the situation where JXcore is blocked by an infinite
...
Number of seconds JXcore has been running.
