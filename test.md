...
The `where` parameter (it corresponds to command line option [--extract-where](#--extract-where)) allows to change a folder name into which the package is extracted.
By default, when the value is not provided, it is set as the name of the package. You may change it into any other name or path, for example:
...
The `what` parameter corresponds to command line option [--extract-what](#--extract-what) with the only difference,
