...
See [`allowMonitoringAPI`](jxconfig.markdown#jxconfig_allowmonitoringapi).
...
(check `start_delay` in [Config file](jxcore-command-monitor.markdown#jxcore_command_monitor_config_file) section).
...
1. The `start_delay` (see [Config file](jxcore-command-monitor.markdown#jxcore_command_monitor_config_file)) parameter is engaged.
