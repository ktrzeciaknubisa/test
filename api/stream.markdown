...
Almost all JXcore programs, no matter how simple, use Streams in some
way.  Here is an example of using Streams in a JXcore program:
...
would be piped into the parser, which is a more idiomatic JXcore stream
approach.
...
cases in JXcore internals where this is done, particularly in the
...
simply discarded.  However, in JXcore and Node v0.10 and beyond, the socket will
...
No streams in JXcore are object mode streams.  This pattern is only
