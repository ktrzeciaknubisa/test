...
(Note: All sockets in JXcore set `SO_REUSEADDR` already)
...
user and used as a client (with `connect()`) or they can be created by JXcore
...
simply might be too slow. JXcore will internally queue up the data written to a
