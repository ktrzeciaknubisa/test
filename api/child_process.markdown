...
JXcore provides a tri-directional `popen(3)` facility through the
`child_process` module.

It is possible to stream data through a child's `stdin`, `stdout`, and
`stderr` in a fully non-blocking way.  (Note that some programs use
line-buffered I/O internally.  That doesn't affect JXcore but it means
...
Also, note that JXcore establishes signal handlers for `'SIGINT'` and `'SIGTERM`',
...
the `message` event, since they are internal messages used by JXcore.
...
2. `'ipc'` - Create an IPC channel for passing messages/file descriptors
   ...
   trigger ChildProcess.on('message').  If the child is a JXcore/Node.js program, then
   the presence of an IPC channel will enable process.send() and
   process.on('message').
3. `'ignore'` - Do not set this file descriptor in the child. Note that JXcore
   will always open fd 0 - 2 for the processes it spawns. When any of these is
   ignored JXcore will open `/dev/null` and attach it to the child's fd.
...
This is a special case of the `spawn()` functionality for spawning JXcore
processes. In addition to having all the methods in a normal ChildProcess
instance, the returned object has a communication channel built-in. See
`child.send(message, [sendHandle])` for details.

These JXcore child processes are still whole new instances of V8. Assume at least 30ms
startup and 10mb memory for each new process. That is, you cannot create many
thousands of them.

The `execPath` property in the `options` object allows for a process to be
created for the child rather than the current `jx` executable. This should be
