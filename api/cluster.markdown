...
A single instance of JXcore runs in a single thread. To take advantage of
multi-core systems the user will sometimes want to launch a cluster of JXcore
...
Running JXcore will now share port 8000 between the workers:

    % NODE_DEBUG=cluster jx server.js
...
efficiently.  There is no routing logic in JXcore, or in your program,
...
continue to accept connections.  JXcore does not automatically manage the
...
* {Object}
  * `execArgv` {Array} list of string arguments passed to the `jx` executable.
...
See [child_process event: 'exit'](child_process.markdown#event--exit).
...
child_process.markdown#child_processforkmodulepath-args-options)
...
see [server.close()](net.markdown#event-close), the IPC channel to the worker
...
it is [disconnect](child_process.markdown#childdisconnect).
